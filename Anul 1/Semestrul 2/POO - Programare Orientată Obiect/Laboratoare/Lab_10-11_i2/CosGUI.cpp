...
void CosGUI::connectSignals() {
	...
	QObject::connect(btnExport, &QPushButton::clicked, this, [this]() {
		std::string numeFisier = editExport->text().toStdString();

		if (numeFisier.empty()) {
			QMessageBox::warning(this, "Atentie", "Introduceti un nume de fisier pentru export!");
			return;
		}
		...
		try {
			...
			QMessageBox::information(this, "Succes", "Cosul a fost exportat cu succes!");
			editExport->clear();
		}
		catch (const std::exception& ex) {
			QMessageBox::warning(this, "Eroare", "Nu s-a putut exporta cosul: " + QString::fromStdString(ex.what()));
		}
		});
}